      ...
      <div className="flex flex-col gap-4 text-lg items-center xxl:items-start">
        ...
        <div className="flex flex-col gap-4">
          {menuList.map((item) => (
            <Link
              href={item.link}
              className="p-1 rounded-full hover:bg-[#181818] flex items-center gap-3"
              key={item.id}
            >
              <Image
                src={`/icons/${item.icon}`}
        ...
        <Link
          href="/compose/post"
          className="hidden xxl:block bg-textWhite text-textBlack rounded-full font-bold py-2 px-20"
        >
          Post
        </Link>
      </div>
